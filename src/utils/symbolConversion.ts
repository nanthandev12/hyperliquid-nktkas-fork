...
import type { PerpsMetaAndAssetCtxs, SpotMetaAndAssetCtxs } from '../types/info/assets.ts';

/** Parameters for the {@linkcode SymbolConversion} constructor. */
export interface SymbolConversionParameters<T extends IRequestTransport = IRequestTransport> {
...
> implements SymbolConversionParameters<T> {
   ...
   * const symbolConverter = new hl.SymbolConversion({ transport });
   * ```
   */
  constructor(args: SymbolConversionParameters<T>) {
  ...
  private async refreshAssetMaps(): Promise<void> {
    try {
      const [perpMeta, spotMeta] = await Promise.all([
        this.transport.request<PerpsMetaAndAssetCtxs>('info', {
          type: 'metaAndAssetCtxs'
        }),
        this.transport.request<SpotMetaAndAssetCtxs>('info', {
